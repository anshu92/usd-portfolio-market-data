...
an adjusted or total-return series. When present, `benchmark-distributions.parquet`,
`benchmark-total-returns.parquet`, and `benchmark-certification.json` are derived from
separately retrieved Yahoo Finance Chart responses whose SHA-256 values, retrieval
times, event lineage, and reconciliation results are recorded in the manifest. Those
optional VTI, SPY, and BIL benchmark rows do not turn the raw
