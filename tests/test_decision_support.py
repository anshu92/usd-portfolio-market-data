...
from decision_support_contract import DATABASE_FILENAME, MANIFEST_FILENAME, PHASES
...
def build_artifact(decision_builder_module, release: Path, output: Path):
...
def test_builds_and_verifies_compact_read_model(
    ...
    packs = {
        phase_id: json.loads(
            (output / "phase-packs" / f"{phase_id}.json").read_text()
        )
        for phase_id in PHASES
    }
    assert packs["accounting"]["status"] == "BLOCKED"
    assert packs["execution_research"]["consumer_live_snapshot_required"] is True
    assert packs["execution_research"]["external_capabilities"][0]["state"] == "CONSUMER_REQUIRED"
    ...
    try:
        ...
        assert connection.execute("SELECT count(*) FROM market_snapshot").fetchone() == (1,)
        assert connection.execute("SELECT count(*) FROM security WHERE is_current=1").fetchone() == (1,)
    finally:
        connection.close()
...
def test_build_is_deterministic_for_identical_inputs(
    ...
    assert hashlib.sha256((first / DATABASE_FILENAME).read_bytes()).digest() == hashlib.sha256(
        (second / DATABASE_FILENAME).read_bytes()
    ).digest()
...
def test_pointer_binds_artifact_run_and_source_release(
    ...
    manifest = build_artifact(decision_builder_module, release_dir, artifact_dir)
    ...
    artifact = {
        "id": 10,
        "name": f"decision-support-{source_tag}",
